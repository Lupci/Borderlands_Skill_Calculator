...
function updateTree(treeHandle) {
	...
	$(treeHandle).find("div.tier").each(function(index) {
		...
		$(this).children("div.skill").each(function(index) {
			...
			totalPoints += p;
			tierTotal += p;
			$(this).children("span.points").html(
				p + "/" + m
			);
			$(this).children("span.points").css("visibility", (totalPoints < 5 * tierLevel) ? "hidden" : "visible");
			$(this).removeClass("partial full");
			if (p != 0) {
				$(this).addClass(p < m ? "partial" : "full");
			}
			$(this).find(".bonusNum").each(function(index) {
	...
	$(treeHandle).parent().children(".color").height(Math.min(80 + totalPoints * 59.0 / 5 + (totalPoints > 25 ? 21 : 0), 391));
}
...
$(document).ready(function () {
	...
	$("div.treeBox").bind("contextmenu", function() { return false; });
